...
def page_not_found(request):
    return render(request, '404.html',{'navs':navbar()})


def page_error(request):
    return render(request, '500.html',{'navs':navbar()})


def permission_denied(request):
    return render(request, '403.html',{'navs':navbar()})
...
def list(request,bigtype='',stype=''):
	...
	return render(request, 'list.html',{'friendlink':friendlink(),'youlike':youlike(),'tagsyun':tagsyun(),'navs':navbar(),"blogs":blog,'category':'category','bigtype':bigtype,'stype':stype,'allPage':range(1,allPage),'maxPage':allPage,'curPage':curPage,'ishot':ishot(),'date_class':date_class()})
